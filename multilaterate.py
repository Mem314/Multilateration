...
class Circle():
    ...
    def find_intercept(self, c2):
        ...
        self.dx, self.dy = s
        self.d = math.sqrt(np.sum(s**2))
        # Test if there is an overlap.  Note: this won't detect if
        # one circle completly surrounds the other.
        if self.d > (self.radius + c2.radius):
            ...
            #cosine rule
            self.cosA = (c2.radius**2 - self.radius**2 + self.d**2)\
                        /(2*c2.radius*self.d)
            self.A = math.acos(self.cosA)

            self.Ia = c2.centre - [math.cos(self.A+self.theta)*c2.radius,
                                   math.sin(self.A+self.theta)*c2.radius]
            self.Ib = c2.centre - [math.cos(self.A-self.theta)*c2.radius,
                                   -math.sin(self.A-self.theta)*c2.radius]
